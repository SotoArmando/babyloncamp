...
function startHandoff(container, unit) {
  ...
  if (kind === "pulse") {
    ...
    const hold = 720;
    const fade = 520;
    const pulseMs = flashes * beat + hold + fade;
    unit.handoffTimer = setTimeout(() => {
      container.classList.add("is-handoff-hold");
      unit.handoffTimer = setTimeout(() => {
        unit.handoffTimer = 0;
        container.classList.add("is-handoff-done");
      }, hold + fade);
    }, flashes * beat);
    return pulseMs;
  }
...
function propPose(action, t, half) {
